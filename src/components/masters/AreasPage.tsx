...
import type { Area } from "../../types/master";
...
export default function AreasPage() {
  const {
    ...
    forceServerSearch,
    isServerSearching,
  } = useMasterData<Area>("areas", true, undefined, true);
  ...
  const handleSearch = (term: string) => {
    setSearch(term);
    const localHits = items.filter((a) => a.name?.toLowerCase().includes(term.toLowerCase()));
  ...
  return (
    <>
      <MasterPageLayout
        title="Areas"
        resource="areas"
        searchPlaceholder="Search areas by name or project..."
      ...
      <Dialog open={isModalOpen} onOpenChange={closeModal}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingItem ? "Edit Area" : "Add New Area"}</DialogTitle>
          </DialogHeader>
          <MasterForm
            resource="areas"
